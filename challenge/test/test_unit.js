...
test('002 double and only return those greater than ten', () => {
  expect(
    challenge.doubleAndGreaterThanTen([1, 2, 4, 8, 16])
  )
  .toEqual([16, 32])
})
...
test('004 total amount pledged', () => {
  ...
  expect(
    challenge.totalAmountPledged(pledges)
  )
  .toEqual(231)
})
...
test('005 category stats sorted by category name', () => {
  ...
  expect(
    challenge.categoryStatsSortedByCategoryName(projects)
  )
  .toEqual([
    new CategoryStats(
      'art',
...
test('006 had project reached goal after each pledge change', () => {
  ...
  expect(
    challenge.hadProjectReachedGoalAfterEachPledgeChange(fixtures.skullGraphicTee, pledgeChanges)
  )
  .toEqual(
...
test('007 compare monthly stats per year', () => {
  ...
  .toEqual(
    [28, 10, -4, -21, 20, 15, 40, 24, -7, 19, -23, 14]
  )
  
  expect(challenge.compareMonthlyStatsPerYear([5, 7], [9, 8, 3]))
    .toEqual([4, 1])
...
test('008 recommend projects from same category', () => {
  expect(
    challenge.recommendProjectsFromSameCategory(
      fixtures.beeAndPuppyCat,
      [fixtures.obviousChild, fixtures.kungFury]
    )
  )
  .toEqual(
  ...
  expect(
    challenge.recommendProjectsFromSameCategory(
      fixtures.beeAndPuppyCat,
      [fixtures.kungFury, fixtures.beeAndPuppyCat]
    )
  )
  .toEqual(
    [fixtures.kungFury]
  ...
  expect(
    challenge.recommendProjectsFromSameCategory(
      fixtures.beeAndPuppyCat,
      [fixtures.kungFury, fixtures.meowWolf]
    )
  )
  .toEqual(
    [fixtures.kungFury]
...
test('009 recommendation feed', () => {
  ...
  expect(
    challenge.recommendationFeed([fixtures.beeAndPuppyCat], fixtures.allProjects)
  )
  .toEqual(
    [fixtures.kungFury, fixtures.obviousChild]
  )
  ...
  .toEqual(
    [fixtures.kungFury, fixtures.miniMuseum, fixtures.obviousChild]
  )
...
test('011 activity feed', () => {
  ...
  expect(
    challenge.activityFeed(activities)
  )
  .toEqual(
    [
      ['like', 1],
